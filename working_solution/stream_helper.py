...
class ShillingStreamHelper:
    ...
    def __init__(self, group_name: str, group_settings: Dict, google_utils):
        ...
        if 'next_launch_delay' not in self.settings:
            self.settings['next_launch_delay'] = 7200  # 2 години в секундах
        ...
        while not self.stop_event.is_set():
            try:
                ...
                for x in accounts_raw:
                    ...
                    if x.get('Next_Launch', 0) <= time.time() or x.get('Next_Launch', 0) == 0:
                        account = Account(
                            username,
                            x.get('Password', ''),
                            x.get('Auth_Token', ''),
                        ...
                        print(f"�� Аккаунт {username} запущено паралельно")
    ...
    async def update_next_launch(self, username):
        ...
        try:
            ...
            next_launch_delay = self.settings.get('next_launch_delay', 7200)  # 2 години за замовчуванням
            next_launch_time = current_time + next_launch_delay
    ...
    def get_active_accounts_info(self) -> List[Dict]:
        ...
        """
        active_accounts_info = []
        for username, (account, task) in self.active_tasks.items():
            account_info = {
                'username': username,
                'status': 'running' if not task.done() else 'completed',
                'started_at': getattr(account, 'start_time', None),
                'task_done': task.done()
            }
            active_accounts_info.append(account_info)
        return active_accounts_info

    def remove_account_from_tasks(self, username: str, auth_token: str = None):
        """Видаляє акаунт з активних завдань"""
        try:
            print(f"🗑️ Видаляємо акаунт {username} з активних завдань stream_helper...")
            
            if username in self.active_tasks:
                account, task = self.active_tasks[username]
                
                # Скасовуємо завдання якщо воно ще виконується
                if not task.done():
                    task.cancel()
                    print(f"✅ Завдання для акаунта {username} скасовано")
                
                # Видаляємо з активних завдань
                del self.active_tasks[username]
                print(f"✅ Акаунт {username} видалено з активних завдань")
                
                print(f"📊 Група {self.group_name}: Активних акаунтів: {len(self.active_tasks)}")
                return True
            else:
                print(f"⚠️ Акаунт {username} не знайдено в активних завданнях")
                return False
                
        except Exception as e:
            print(f"❌ Помилка при видаленні акаунта {username} з stream_helper: {e}")
            return False
